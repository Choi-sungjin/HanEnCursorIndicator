...
| 말풍선 | 화면 한 번 읽기, 상시 읽기, 정지·숨기기, 답변 지침, 글꼴·글자 크기·색상, 화면 읽기 단축키, 말풍선 음성 읽기·전용 단축키 |
...
트레이의 상시 화면 읽기 토글을 켜면 즉시 한 번 읽고 이후 20초 간격으로 읽는다. 처리 중에는 중복 요청하지 않는다. 끄면 진행 요청을 취소하며, 앱 재시작 시에는 꺼짐으로 시작한다.
...
- 트레이 화면 읽기는 클릭 당시 커서가 있는 모니터를 한 번 캡처한다.
...
- 요청은 한 번에 하나이며 자동 재시도하지 않는다. 화면 읽기의 전체 요청 제한은 180초, 일반 대화는 60초다.
